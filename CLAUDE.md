...
**Data layer**: `CosmosDbClient` is a manual singleton that holds references to nine Cosmos DB containers: `comics`, `images`, `users`, `sessions`, `carts`, `discounts`, `archived-orders`, `return-events`, `audit-logs`. The database is `comic-db` in Azure.
...
**Cart grouping**: `CartComponent` and `AdminOrdersComponent` both use a `groupedRows(): CartRow[]` computed property that collapses flat `CartItem[]` lists into typed display rows (`type: 'single' | 'set'`). Set rows include all member comics under one header.
